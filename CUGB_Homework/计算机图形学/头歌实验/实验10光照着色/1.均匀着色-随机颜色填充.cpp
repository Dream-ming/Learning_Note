...
int main(int argc, char** argv)
{
    ...
    Matrix ModelView = lookat(eye, center, up) * rotation_y(15)  ;
	/********** End **********/
    ...
    for (int i = 0; i < model->nfaces(); i++)
    {
        vector<int> face = model->face(i);
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                triangleVertex[j] = ViewPort * Projection * ModelView * v1;
            }
			// 请补充代码，其中每个面颜色随机生成
			/********** Begin ********/
            triangle( triangleVertex, zbuffer, image, PNGColor(rand() % 255, rand() % 255, rand() % 255, 255));
			/********** End **********/
    }
