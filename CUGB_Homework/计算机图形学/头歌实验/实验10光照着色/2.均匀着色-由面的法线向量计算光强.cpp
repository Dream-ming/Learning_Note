...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < model->nfaces(); i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			screen_coords[j] = ViewPort * Projection * ModelView * world_coords[j];
		}
		// 每个面的颜色根据该面的法向与入射光的夹角来计算
		/********** Begin ********/
        ...
        PNGColor clr = PNGColor(intensity * 255, intensity * 255, intensity * 255, 255);
