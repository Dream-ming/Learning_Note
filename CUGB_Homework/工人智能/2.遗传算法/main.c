...
int main() {                     // 主函数；
    ...
    printf("The Algorithm is based on the function y = -x^2 + 5 to find the "
...
int x(chrom popcurrent) {   // 函数：将二进制换算为十进制；
  // 此处的染色体长度为，其中个表示符号位
    ...
    int z;
    z = (popcurrent.bit[0] * 1) + (popcurrent.bit[1] * 2) +
        (popcurrent.bit[2] * 4) + (popcurrent.bit[3] * 8) +
        (popcurrent.bit[4] * 16);
...
void *crossover(chrom popnext[4]) {    // 函数：交叉操作；
    ...
    for (i = 0; i < 4; i++) {
        printf(
            "\nCrossOver popnext[%d]=%d%d%d%d%d%d    value=%d    fitness = %d",
            i, popnext[i].bit[5], popnext[i].bit[4], popnext[i].bit[3],
            popnext[i].bit[2], popnext[i].bit[1], popnext[i].bit[0],
...
void *mutation(chrom popnext[4]) {    // 函数：变异操作；
    ...
    if (random == 25) {    // random==25的概率只有2%，即变异率为，所以是以小概率进行变异！！
        ...
        value = x(popnext[row]);
        printf("\nMutation occured in popnext[%d] bit[%d]:=%d%d%d%d%d%d    "
               "value=%d   fitness=%d",
               row, col, popnext[row].bit[5], popnext[row].bit[4],
               popnext[row].bit[3], popnext[row].bit[2], popnext[row].bit[1],
